...
function TicketData({ guest, configData }) {
  const URL = `https://arin.miftahussalam.com?to=${encodeURIComponent(guest.name).replace(
    /%20/g,
    '+',
  )}&type=invitation&code=${guest.code}`;
  return (
    <div css={styWrapper}>
      <div class="container">
        <div class="ticket">
        ...
        <div class="details">
          ...
          {guest.desc && <div class="tdata">{guest.desc}</div>}
          {!guest.desc && <div class="tdata">-</div>}
          <div class="tinfo">Waktu</div>
          <div class="tdata">11.00 s/d 15.00 WIB</div>
          {/* attendance */}
          {guest.isAttended ? (
            <>
          ...
          <div class="tdata additional">
            <ul style={{ paddingLeft: '16px' }}>
              <li>QRCode berisi link e-invitation</li>
              <li>
